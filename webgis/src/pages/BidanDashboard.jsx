...
export default function BidanDashboard() {
    ...
    // State for Tabs
    const [activeTab, setActiveTab] = useState('posyandu'); // 'posyandu', 'anak', 'kader'
    ...
    const [kaderForm, setKaderForm] = useState({ name: '', email: '', password: '', no_hp: '', posyandu_ids: [], desa_id: '' });
    ...
    const fetchData = async () => {
        try {
            ...
            dashData.user.lokasi = 'Bidan Desa';
        ...
        <div className="bg-background text-on-background font-body-md antialiased flex h-screen overflow-hidden">
            ...
            <aside className="w-64 bg-surface border-r border-outline-variant/30 hidden md:flex flex-col z-40 shadow-sm">
                <div className="p-6 border-b border-outline-variant/30 text-center">
                    ...
                    <h2 className="font-headline-md text-primary font-bold">{data.user.name || "Bidan"}</h2>
                    <p className="font-body-md text-on-surface-variant text-sm">Bidan Desa</p>
                    <p className="text-xs text-on-surface-variant mt-1 font-bold">{data.user.puskesmas_nama ? `Puskesmas ${data.user.puskesmas_nama.replace('Puskesmas ', '')}` : 'Belum ditugaskan ke Puskesmas'}</p>
                </div>
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
                ...
                <header className="md:hidden bg-surface/90 backdrop-blur-md border-b border-outline-variant/50 sticky top-0 z-40 py-4 px-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="font-headline-md font-bold text-primary">Bidan Dashboard</h1>
                    </div>
                </header>
                            ...
                            <div className="flex flex-col gap-4 animate-fade-in-up">
                                <div className="flex justify-between items-center bg-surface p-4 rounded-2xl border border-outline-variant/50 shadow-sm">
                                    <div className="flex items-center gap-3 text-on-surface-variant">
                                        ...
                                        <div>
                                            <p className="font-bold text-on-surface">Total Puskesmas</p>
                                            <p className="text-sm">{puskesmasOptions.length} puskesmas di kabupaten ini</p>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-surface-container-low border-b border-outline-variant/50">
                                                <th className="py-3 px-4 font-label-sm text-on-surface-variant font-bold w-16 text-center">No</th>
                                                <th className="py-3 px-4 font-label-sm text-on-surface-variant font-bold">Nama Puskesmas</th>
                                                <th className="py-3 px-4 font-label-sm text-on-surface-variant font-bold">Status</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {puskesmasOptions.map((puskesmas, index) => (
                                                <tr key={puskesmas.id} className="border-b border-outline-variant/30 hover:bg-surface-container-lowest transition-colors">
                                                    <td className="py-3 px-4 font-body-sm text-on-surface-variant text-center">{index + 1}</td>
                                                    <td className="py-3 px-4 font-title-sm text-on-surface">
                                                        <div className="font-bold">{puskesmas.nama}</div>
                                                        {data.user.region_names && (
                                                            <div className="text-xs text-on-surface-variant mt-0.5">
                                                                Desa {data.user.region_names.desa}, Kec. {data.user.region_names.kecamatan}, Kab. {data.user.region_names.kabupaten}, {data.user.region_names.provinsi}
                                                            </div>
                                                        )}
                                                    </td>
                                                    <td className="py-3 px-4 font-body-sm">
                                                        <span className="bg-success/20 text-success px-2 py-0.5 rounded-full text-[11px] font-bold">
                                                            Aktif
                                                        </span>
                                                    </td>
                                                </tr>
                                            ))}
                                            {puskesmasOptions.length === 0 && (
                                                <tr><td colSpan="3" className="py-8 text-center text-on-surface-variant">Belum ada data puskesmas.</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        {activeTab === 'posyandu' && (
                            <div className="flex flex-col gap-4 animate-fade-in-up">
                                <div className="flex justify-between items-center bg-surface p-4 rounded-2xl border border-outline-variant/50 shadow-sm">
                                    ...
                                    <button onClick={() => { setEditPosyanduData(null); setPosyanduForm({nama:'', alamat:'', puskesmas_id: data.user?.puskesmas_id || '', desa_id: data.user?.desa_id || ''}); setIsPosyanduModalOpen(true); }} className="px-4 py-2.5 bg-primary text-on-primary rounded-xl font-label-md flex items-center gap-2 hover:bg-primary/90 shadow-md">
                        ...
                        {activeTab === 'kader' && (
                            <div className="flex flex-col gap-4 animate-fade-in-up">
                                <div className="flex justify-between items-center bg-surface p-4 rounded-2xl border border-outline-variant/50 shadow-sm">
                                    ...
                                    <button onClick={() => { setEditKaderData(null); setKaderForm({name:'', email:'', password:'', no_hp:'', posyandu_ids:[], desa_id: data.user?.desa_id || ''}); setIsKaderModalOpen(true); }} className="px-4 py-2.5 bg-primary text-on-primary rounded-xl font-label-md flex items-center gap-2 hover:bg-primary/90 shadow-md">
                                        ...
                                        <tbody>
                                            {kaderList.map(kader => (
                                                <tr key={kader.id} className="border-b border-outline-variant/30 hover:bg-surface-container-lowest transition-colors">
                                                    ...
                                                    <td className="py-3 px-4 flex justify-end gap-2">
                                                        <button onClick={() => { setEditKaderData(kader); setKaderForm({name: kader.name, email: kader.email, password: '', no_hp: kader.no_hp || '', posyandu_ids: kader.posyandu_list.map(p => p.id), desa_id: kader.desa_id || ''}); setIsKaderModalOpen(true); }} className="w-8 h-8 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center hover:bg-secondary hover:text-on-secondary shadow-sm">
                                                            <span className="material-symbols-outlined text-[18px]">edit</span>
                                                        </button>
            ...
            <div className={`md:hidden fixed inset-0 z-50 transition-all duration-300 ease-in-out ${isDrawerOpen ? 'bg-on-background/50 backdrop-blur-sm opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsDrawerOpen(false)}>
                <aside onClick={(e) => e.stopPropagation()} className={`h-full w-72 bg-surface shadow-2xl flex flex-col py-4 transition-transform duration-300 ease-out transform ${isDrawerOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    <div className="px-4 pb-6 border-b border-outline-variant flex flex-col gap-2 relative">
                        ...
                        <h2 className="font-headline-md text-primary font-bold">{data.user.name || "Bidan"}</h2>
                        <p className="font-body-md text-on-surface-variant">Bidan Desa</p>
                        <p className="text-xs text-on-surface-variant mt-1 font-bold">{data.user.puskesmas_nama ? `Puskesmas ${data.user.puskesmas_nama.replace('Puskesmas ', '')}` : 'Belum ditugaskan ke Puskesmas'}</p>
                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            {data.user?.desa_list?.length > 0 ? (
                                                data.user.desa_list.map(d => (
                                                    <option key={d.id} value={d.id}>{d.nama}</option>
                                                ))
                                            ) : (
                                                <option value={data.user?.desa_id || ''}>{data.user?.region_names?.desa || 'Desa tidak tersedia'}</option>
                                            )}
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div className="p-6">
                            <form onSubmit={handleSavePosyandu} className="flex flex-col gap-5">
                                <div className="p-3 bg-tertiary-container/30 border border-tertiary-container rounded-xl flex flex-col gap-1 mb-2">
                                    ...
                                    <p className="text-sm text-on-surface">Posyandu akan didaftarkan pada wilayah kewenangan Anda secara otomatis (Provinsi, Kabupaten, Kecamatan, Desa Anda saat ini).</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <select 
                                            value={posyanduForm.puskesmas_id} 
                                            onChange={e => setPosyanduForm({...posyanduForm, puskesmas_id: e.target.value})} 
                                            className={`w-full pl-9 pr-4 py-2.5 border border-outline-variant rounded-xl appearance-none outline-none focus:border-primary transition-colors ${data.user?.puskesmas_id ? 'bg-surface-variant text-on-surface-variant opacity-70 cursor-not-allowed' : 'bg-surface'}`}
                                            disabled={!!data.user?.puskesmas_id}
                                        >
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <select 
                                            required
                                            value={posyanduForm.desa_id} 
                                            onChange={e => setPosyanduForm({...posyanduForm, desa_id: e.target.value})}
                                            className="w-full pl-9 pr-4 py-2.5 bg-surface border border-outline-variant rounded-xl appearance-none outline-none focus:border-primary transition-colors"
                                        >
                                            <option value="">-- Pilih Desa --</option>
                                            {data.user?.desa_list?.length > 0 ? (
